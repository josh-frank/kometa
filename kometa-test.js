...
const fs = require("fs");
const { KometaSession } = require("./kometa-workspace.js");
...
// Encode
new KometaSession()
  .generatePad(PAD, MESSAGE)
  .encode(COVER, MESSAGE, ENCODED)
  // session goes out of scope — pad still on disk for the receiver

// Decode (separate session: simulates receiver side)
new KometaSession()
  .usePad(PAD)
  .decode(ENCODED, DECODED);

// Verify
const result = fs.readFileSync(DECODED, "utf8").trimEnd();
