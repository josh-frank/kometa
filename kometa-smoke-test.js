...
// Write temp files
writeText("/tmp/kometa_cover.txt", cover);
writeText("/tmp/kometa_message.txt", secret);
writeText("/tmp/kometa_pad.txt", pad);

// Roundtrip
encode(
  "/tmp/kometa_cover.txt",
  "/tmp/kometa_message.txt",
  "/tmp/kometa_pad.txt",
);

// Restore pad for decode (destroyed by decode, so write fresh copy)
writeText("/tmp/kometa_pad.txt", pad);
const result = decode("/tmp/kometa_cover.txt.out", "/tmp/kometa_pad.txt");

console.log("Message:", result);
console.log("Match:  ", result === secret ? "✓" : "✗ MISMATCH");
